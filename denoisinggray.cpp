...
int main(int argc, char *argv[])
{
  ...
  // Allocate image data array for original image
  ...
  unsigned char ImageDifference[SizeH][SizeW][1];
  for(int i = 0; i < SizeH; i++) {
    for (int j = 0; j < SizeW; j++) {
      ImageDifference[i][j][0] = ImagedataOrig[i][j][0] - Imagedata[i][j][0];
    }
  }

  if (!(file = fopen("DifferenceImage.raw", "wb"))) {
    cout << "Cannot open file: " << "DifferenceImage.raw" << endl;
    exit(1);
  }
  fwrite(ImageDifference, sizeof(unsigned char), SizeH * SizeW * BytesPerPixel, file);
  fclose(file);
